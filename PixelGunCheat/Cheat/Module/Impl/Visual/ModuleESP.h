...
static BKCModule __esp = { "ESP", "[THIS MODULE IS CURRENTLY DISABLED, AND WILL NOT DO ANYTHING] I swear it's just a visual glitch in OBS!", VISUAL, 0x0, true, { &__esp_style, &__esp_thickness, &__esp_corner_rounding, &__esp_teammates, &__esp_tracers, &__esp_tracers_screen_pos, &__esp_rainbow, &__esp_rgb_speed } };
...
public:
    ...
    static void add_esp(void* player)
    {
        try
        {
            // TODO: FIX CRASH SOMEWHERE IN HERE ASAP PLS
            if (player == nullptr || Hooks::our_player == nullptr || Hooks::main_camera == nullptr) return;
